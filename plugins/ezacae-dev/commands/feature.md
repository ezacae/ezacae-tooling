...
2. **Conception → implémentation → revue** → Mike invoque automatiquement `/sarah <TICKET>`.
   Sarah enchaîne chuck (conception) → developer (implémentation, branche + tests + MR) → `pr-review-toolkit:code-reviewer` (revue), en synchronisant le statut JIRA et en attachant les livrables.
